...
bool AnalyzerWorkerThread::run(uint32_t coreId)
{
    ...
    centers = torch::zeros({(long) p_learner->get_K(), (long) (p_analyzer_conf->n_fft / 2) + 1});
...
void AnalyzerWorkerThread::wave_analyze()
{
    ...
    for (iter_mp = mp.cbegin(); iter_mp != mp.cend();) {
        ...
        for (size_t i = _ve.size() - 1; i > 0; i --) {
            // LOGF("ts: %f", raw_data[_ve[i]].time_stamp);
            // LOGF("ts-1: %f", raw_data[_ve[i - 1]].time_stamp);
            // LOGF("sub: %f", raw_data[_ve[i]].time_stamp - raw_data[_ve[i - 1]].time_stamp);
        ...
        if (m_is_train) {
            ...
            if (ten_res.size(0) > p_analyzer_conf->mean_win_train + 1
                ...
                for (size_t i = 0; i < p_analyzer_conf->num_train_sample; i ++) {
                    size_t start_index = rand() % (ten_res.size(0) - 1 - p_analyzer_conf->mean_win_train);
                    ten_temp = ten_res.slice(0, start_index, start_index + p_analyzer_conf->mean_win_train).mean(0);
                    vector<double_t> _dt;
        ...
        if (ten_res.size(0) > p_analyzer_conf->mean_win_test) {
            ...
            for (size_t i = 0; 
                i + p_analyzer_conf->mean_win_test < ten_res.size(0); 
                i += p_analyzer_conf->mean_win_test) {
                torch::Tensor tt =  ten_res.slice(0, i, i + p_analyzer_conf->mean_win_test).mean(0);
        ...
        if (p_analyzer_conf->ip_verbose) {
            if (p_analyzer_conf->verbose_ip_target.length() != 0 && 
                pcpp::IPv4Address(htonl(iter_mp->first)) == pcpp::IPv4Address(p_analyzer_conf->verbose_ip_target)) {
...
auto AnalyzerWorkerThread::save_res_json() const -> bool 
{
    ...
    ostringstream oss;
    oss << p_analyzer_conf->save_dir << 
            p_analyzer_conf->save_file_prefix << '_' << (int) getCoreId() << ".json";
    ...
    try {
        ...
        if (jin.count("verbose_ip_target")) {
            p_analyzer_conf->verbose_ip_target = 
                static_cast<decltype(p_analyzer_conf->verbose_ip_target)>(jin["verbose_ip_target"]);
        ...
        if (jin.count("verbose_center_core")) {
            p_analyzer_conf->verbose_center_core = 
                static_cast<decltype(p_analyzer_conf->verbose_center_core)>(jin["verbose_center_core"]);
        }
